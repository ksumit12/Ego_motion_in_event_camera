#!/usr/bin/env python3
"""
Create 5-second windows and generate predictions for dt = 1..20 ms (step 1 ms).

This is the 5s analogue of create_short_windows_and_predictions.py
(10 ms windows), and the coarse-dt analogue of create_fine_windows_and_predictions.py
(0–3 ms fine resolution over 5 s).

Output layout
  /media/sumit/New Volume/standard_dt_5s_window/
    window_1_<t0>s_to_<t1>s/
      real_events.npy
      pred_events_dt_01.0ms.npy, pred_events_dt_02.0ms.npy, ..., pred_events_dt_20.0ms.npy
...
OUTPUT_DIR = "/media/sumit/New Volume/standard_dt_5s_window"
...
# Standard dt sweep: 1..20 ms (step 1)
DT_VALUES_MS = np.arange(1.0, 21.0, 1.0)
...
# Main processing
# ------------------------------------------------------------
def process_windows(events_file: str,
                    tracker_file: str,
                    output_dir: str,
                    num_windows: int,
                    window_duration_s: float,
    ...
    for wi, (w0, w1) in enumerate(windows, 1):
        ...
        for dt_ms in DT_VALUES_MS:
            ...
            # Standard dt: format as NN.0ms (01.0ms ... 20.0ms)
            fname = f"pred_events_dt_{int(dt_ms):02d}.0ms."
            pred_path = os.path.join(wdir, fname + ("npz" if use_compressed else "npy"))
            if use_compressed:
            ...
            if save_combined:
                ...
                comb_path = os.path.join(wdir, f"combined_events_dt_{int(dt_ms):02d}.0ms." + ("npz" if use_compressed else "npy"))
                if use_compressed:
                    np.savez_compressed(comb_path, combined=comb)
                else:
                    np.save(comb_path, comb)

    print(f"\nDone. Standard-dt (1..20ms) windows saved to: {output_dir}")


def main():
    p = argparse.ArgumentParser(description="Create 5s windows and generate dt=1..20ms predictions")
    ...
    process_windows(events_file=args.events,
