#!/usr/bin/env python3
"""
Auto-generate short time-window predictions at good times in the recording.

What it does
- Scans the full real-events CSV and tracker CSV
- Finds K short windows (default: 3) of duration 10 ms with high event density
  and spaced apart (min gap configurable)
- For each window, generates per-dt predictions for a fine dt sweep (0–3 ms)
- Saves outputs on the New Volume drive in a folder named 'short_window_predictions'

Output layout
  /media/sumit/New Volume/short_window_predictions/
    window_1_<t0>s_to_<t1>s/
      real_events.(npy|npz)
      pred_events_dt_0.1ms.npy  ... (fractional names preserved)

Note: We save only per-dt predictions and the real events once per window to
limit disk usage. Enable combined saving via --save-combined if needed.
"""
...
WINDOW_DURATION_S = 0.010  # 10 ms short windows
NUM_WINDOWS = 3            # number of windows to select
MIN_GAP_S = 0.5            # enforce spacing between selected windows

# Standard dt sweep: 1 ms to 20 ms (step 1 ms)
DT_VALUES_MS_PRIMARY = np.arange(1.0, 21.0, 1.0)
DT_VALUES_MS_TAIL = np.array([], dtype=float)
...
def select_dense_windows(events: np.ndarray,
    ...
    """Pick windows by event density with spacing.

    - Bin events at 1 ms resolution to estimate local densities
    - Slide a 10 ms window to get event counts
    - Pick top-K windows, enforcing a minimum temporal gap
    """
    t = events[:, 3].astype(np.float64)
    if len(t) == 0:
        return []
    t0, t1 = float(t.min()), float(t.max())
    bin_dt = 0.001  # 1 ms bins
    nbins = int(np.ceil((t1 - t0) / bin_dt)) + 1
    hist, edges = np.histogram(t, bins=nbins, range=(t0, t0 + nbins * bin_dt))
    # rolling sum over window_duration_s
    k = int(round(window_duration_s / bin_dt))
    if k <= 0:
        k = 1
    csum = np.cumsum(np.pad(hist, (1, 0)))
    window_counts = csum[k:] - csum[:-k]
    # indices to candidate windows (left-edge bin indices)
...
# Main processing
# ------------------------------------------------------------
def process_short_windows(events_file: str,
                          tracker_file: str,
                          output_dir: str,
                          num_windows: int,
                          window_duration_s: float,
    ...
    print(f"\nDone. Short-window predictions saved to: {output_dir}")


def main():
    p = argparse.ArgumentParser(description="Create short-window predictions automatically")
    ...
    process_short_windows(events_file=args.events,
